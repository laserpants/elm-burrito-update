...
The main takeaway here is that the `Update` is as a type alias for the usual `( model, cmd )` tuple. However, as the extra `a` type parameter suggests, there is a bit more going on. More about this in due time.
